...
extension ViewController {
    ...
    func placeDisk(_ disk: Disk, atX x: Int, y: Int, animated isAnimated: Bool, completion: ((Bool) -> Void)? = nil) throws {
        let diskCoordinates = Reversi.flippedDiskCoordinatesByPlacingDisk(disk, boardView, atX: x, y: y)
...
extension ViewController {
    ...
    func updateCountLabels() {
        for side in Disk.sides {
            countLabels[side.index].text = "\(Reversi.countDisks(of: side, boardView))"
        }
    }
    ...
    func updateMessageViews() {
        ...
        case .none:
            if let winner = Reversi.sideWithMoreDisks(boardView) {
                messageDiskSizeConstraint.constant = messageDiskSize
